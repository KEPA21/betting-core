...
def _ensure_fk_exists_for_predictions(db: Session, rows: list[dict]) -> None:
    ...
    if model_ids:
        stmt = text(
            "SELECT model_id FROM core.models WHERE model_id IN :ids"
...
def get_predictions(
    ...
    params: dict = {}
    if match_id:
        where.append("match_id = :match_id")
    ...
    if selection_id:
        where.append("selection_id = :selection_id")
        params["selection_id"] = selection_id
    if ts_from:
        where.append("predicted_at >= :ts_from")
        params["ts_from"] = ts_from
    if ts_to:
        where.append("predicted_at <= :ts_to")
        params["ts_to"] = ts_to
    ...
    total = db.execute(
        text(f"SELECT COUNT(*) FROM core.predictions WHERE {where_base_sql}"), params
    ).scalar_one()
    ...
    items = [
        {
            ...
            "predicted_at": r[
                "predicted_at"
            ].isoformat(),  # låt Pydantic serialisera datetime
        }
        for r in rows
    ]
