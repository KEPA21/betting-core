...
if settings.RATE_LIMIT_ENABLED:
    _limit_odds_per_key = per_key_limiter("odds_post", capacity=settings.RL_ODDS_PER_KEY_CAP, refill_per_sec=settings.RL_ODDS_PER_KEY_REFILL)
    _limit_odds_global = global_limiter("odds_post", capacity=settings.RL_ODDS_GLOBAL_CAP, refill_per_sec=settings.RL_ODDS_GLOBAL_REFILL)
else:
...
def _ensure_fk_exists_for_odds(db: Session, rows: list[dict]) -> None:
    ...
    if bm_ids:
        stmt_bm = (
            text("SELECT bookmaker_id FROM core.bookmakers WHERE bookmaker_id IN :ids")
    ...
    if sel_ids:
        stmt_sel = (
            text("SELECT selection_id FROM core.selections WHERE selection_id IN :ids")
            .bindparams(bindparam("ids", expanding=True))
        )
...
def get_odds(
    ...
    if match_id:
        where_base.append("match_id = :match_id"); params["match_id"] = match_id
    if bookmaker_id:
        where_base.append("bookmaker_id = :bookmaker_id"); params["bookmaker_id"] = bookmaker_id
    if selection_id:
        where_base.append("selection_id = :selection_id"); params["selection_id"] = selection_id
    if ts_from:
        where_base.append("captured_at >= :ts_from"); params["ts_from"] = ts_from
    if ts_to:
        where_base.append("captured_at <= :ts_to"); params["ts_to"] = ts_to   # <-- fix

    # total: utan cursor
    where_base_sql = " AND ".join(where_base)
    total = db.execute(
        text(f"SELECT COUNT(*) FROM core.odds WHERE {where_base_sql}"),
        params
    ).scalar_one()
    ...
    return {"items": items, "total": total, "next_cursor": next_cursor, "next_offset": next_offset}