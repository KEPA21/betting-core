...
def _ensure_fk_exists_for_bets(db: Session, rows: list[dict]) -> None:
    ...
    if bm_ids:
        stmt = text("SELECT bookmaker_id FROM core.bookmakers WHERE bookmaker_id IN :ids").bindparams(bindparam("ids", expanding=True))
        existing = {x[0] for x in db.execute(stmt, {"ids": bm_ids})}
        miss = [str(x) for x in bm_ids if x not in existing]
        if miss: missing.append(f"unknown bookmaker_id(s): {', '.join(miss)}")

    if sel_ids:
        stmt = text("SELECT selection_id FROM core.selections WHERE selection_id IN :ids").bindparams(bindparam("ids", expanding=True))
        existing = {x[0] for x in db.execute(stmt, {"ids": sel_ids})}
        miss = [str(x) for x in sel_ids if x not in existing]
        if miss: missing.append(f"unknown selection_id(s): {', '.join(miss)}")
...
@router.post(
        ...
        responses={**DEFAULT_ERROR_RESPONSES, 201: {"description": "OK"}, 200: {"description": "Idempotent duplicate"}},
...
def post_bet(payload: BetIn, db: Session = Depends(get_db), response: Response = None):
    ...
    if response is not None:
        response.status_code = status.HTTP_201_CREATED if result.get("created") else status.HTTP_200_OK                 
    return result
...
def get_bets(
    ...
    bet_status: Optional[Literal["open", "won", "lost", "void","settled"]] = Query(None, alias="status"),
