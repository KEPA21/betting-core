...
def test_predictions_get_reader_ok(client, reader_headers):
    ...
    if body["items"] and body.get("next_cursor"):
        r2 = client.get(f"/predictions?match_id=m1&limit=1&cursor={body['next_cursor']}", headers=reader_headers)
...
def test_predictions_post_422_validation_error(client, writer_headers):
    ...
    r = client.post("/predictions", headers=writer_headers, json={"items":[{"match_id": "m1"}]})
