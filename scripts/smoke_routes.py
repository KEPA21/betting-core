...
async def main():
    async with httpx.AsyncClient(timeout=30) as c:
        ...
        ok = r.status_code == 200 and {"items","total","next_offset"} <= set(r.json().keys())
        pr(ok, "GET /odds reader", f"{r.status_code} body_keys={list(r.json().keys()) if r.headers.get('content-type','').startswith('application/json') else 'n/a'}")
        ...
        bad = {"items":[{"match_id":"m1","bookmaker_id":"not-a-uuid","selection_id":"not-a-uuid","price":"NaN"}]}
        r = await c.post(f"{API}/odds", headers=W, json=bad)
        j = r.json() if r.headers.get("content-type","").startswith("application/json") else {}
        pr(r.status_code == 422 and j.get("code")=="validation_error", "POST /odds writer 422", str(j.get("code")))

        # 5) 404 på POST /odds writer med fejkade FK
        fake = {"items":[{"match_id":"m404","bookmaker_id":"00000000-0000-0000-0000-000000000000","selection_id":"00000000-0000-0000-0000-000000000000","price":2.0,"captured_at":"2025-08-18T12:00:00Z"}]}
        r = await c.post(f"{API}/odds", headers=W, json=fake)
        j = r.json() if r.headers.get("content-type","").startswith("application/json") else {}
        pr(r.status_code == 404 and j.get("code")=="not_found", "POST /odds writer 404", str(j.get("code")))

        # 6) (Valfri) POSITIV POST /odds – kräver att du sätter BK_ID/SEL_ID i env
        if BK and SEL:
            ok_item = {"items":[{"match_id":"mPOS","bookmaker_id":BK,"selection_id":SEL,"price":2.34,"captured_at":"2025-08-18T12:00:00Z"}]}
            r = await c.post(f"{API}/odds", headers=W, json=ok_item)
            j = r.json()
            pr(r.status_code in (200,201) and set(j.keys()) >= {"inserted","updated"}, "POST /odds writer positive", f"{j}")
        else:
        ...
        pr(r.status_code in (200,404,204), "GET /predictions reader", str(r.status_code))

        # 8) POST /predictions – 404 på fejkade FK
        pred_fake = {"items":[{"match_id":"mX","model_id":"00000000-0000-0000-0000-000000000000","selection_id":"00000000-0000-0000-0000-000000000000","probability":0.55,"predicted_at":"2025-08-18T12:00:00Z"}]}
        ...
        bet_fake = {"external_id":"t-1","user_ref":"u1","match_id":"m1","bookmaker_id":"00000000-0000-0000-0000-000000000000","selection_id":"00000000-0000-0000-0000-000000000000","stake":100,"price":2.1,"placed_at":"2025-08-18T12:30:00Z","idempotency_key":"k-1"}
