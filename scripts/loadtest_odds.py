...
start_ts = datetime.fromisoformat(START_ISO) if START_ISO else datetime.now(tz=timezone.utc)
...
async def main():
    ...
    for b in range(batches):
        jobs.put_nowait({
            "offset": b*BATCH,
            "size": min(BATCH, TOTAL - b*BATCH) 
        })
    
    async with httpx.AsyncClient() as client:
        results = []
        workers = [asyncio.create_task(worker(client, jobs, results)) for _ in range(CONC)]
        t0 = time.perf_counter()
        await jobs.join()
        for _ in workers:
            await jobs.put(None)
    ...
    print(f"Total time: {total_s:.2f}s, RPS (items): {TOTAL/total_s:.0f}s, worst batch: {worst:.2f}s")
