...
def upgrade():
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["market_id"], ["core.markets.market_id"], ondelete="CASCADE"
        ),
        schema="core",
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "probability >= 0 AND probability <= 1", name="chk_predictions_prob_0_1"
        ),
        sa.ForeignKeyConstraint(["model_id"], ["core.models.model_id"]),
        sa.ForeignKeyConstraint(["selection_id"], ["core.selections.selection_id"]),
        sa.UniqueConstraint(
            "match_id", "model_id", "version", "selection_id", name="uq_predictions_key"
        ),
        schema="core",
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "payout IS NULL OR payout >= 0", name="chk_bets_payout_ge_0"
        ),
        sa.ForeignKeyConstraint(["bookmaker_id"], ["core.bookmakers.bookmaker_id"]),
        sa.ForeignKeyConstraint(["selection_id"], ["core.selections.selection_id"]),
...
def downgrade():
    ...
    op.drop_index(
        "uq_selections_market_code_line", table_name="selections", schema="core"
    )
